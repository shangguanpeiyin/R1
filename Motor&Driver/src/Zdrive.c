...
// 		Zdrive[i].Status.FindZeroCnt = 0;
// 		Zdrive[i].Argum.stuckcnt	 = 0;
// 	}
// };
...
// // 寻零函数
// void Zdrive_FindingZero(u8 id)	  // id为 1 - USE_ZDRIVE_NUM
// {
// 	if (Zdrive[id - 1].modeRead != Zdrive_Speed)	// 以速度模式寻零
// 		Zdrive_SetMode((float) Zdrive_Speed, id);
// 	Zdrive_SetSpeed(Zdrive[id - 1].Argum.FindZeroSpeed, id);

// 	// PEAK(motor->ValueSet.current, motor->Limit.ZeroCurrent)	   // 如何在Zdrive中设置寻零电流不要过大？？？？

// 	if (ABS(Zdrive[id - 1].ValueReal.angle - Zdrive[id - 1].ValuePre.angle)
// 		< Zdrive_Zero_THRESHOLD) {	  // 累计小于门槛后，视为寻零成功
// 		if (Zdrive[id - 1].Status.FindZeroCnt++ > 100) {
// 			Zdrive[id - 1].Status.FindZeroCnt = 0;

// 			Zdrive[id - 1].begin			  = false;

// 			Zdrive_SetZero(id);	   // 当前位置再设置为零点
// 		}
// 	} else
// 		Zdrive[id - 1].Status.FindZeroCnt = 0;	  // 防止 积累偶然成功
// }
...
// {
// 	int id = FireArmId - 1;	   // 这里就用了一个电机，不必for循环了。同样防止报错："未用编号的电机发生断连"。
// 	{
// 		if (Zdrive[id].enable) {
// 			if (Zdrive[id].begin) {
...
// 							// 		Zdrive[id].PVTMode.PVTModeFlag	 = false;
// 							// 		FirstFlag						 = 0;
// 							// 		Zdrive[id].ValueSet.angle		 = Zdrive[id].PVTMode.TargetAngle[Zdrive[id].PVTMode.PVTStage]
// 							// 									/ Zdrive[id].Param.ReductionRatio;
// 							// 		Zdrive_SetAcceleration((float) 200, id + 1);
// 							// 		Zdrive_SetDeceleration((float) 100, id + 1);
// 							// 	}
// 							// }
// 						}
// 						break;
// 					}
// 					case Zdrive_FindZero:
// 						Zdrive_FindingZero(id + 1);
// 						break;
// 					default:
// 						break;
// 				}
...
// 	Zdrive[NodeID - 1].Argum.DuanLianCnt = 0;	 // 未断连
// };
#include "Zdrive.h"
